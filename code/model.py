    ...
    with tf.name_scope('edgeconv'):
        ...
        for idx, channel in enumerate(channels):
            x = keras.layers.Conv2D(channel, kernel_size=(1, 1), strides=1, data_format='channels_last',
                                    use_bias=False if with_bn else True, kernel_initializer='he_uniform', name='%s_conv%d' % (name, idx))(x)
            if with_bn:
        ...
        sc = keras.layers.Conv2D(channels[-1], kernel_size=(1, 1), strides=1, data_format='channels_last',
                                 use_bias=False if with_bn else True, kernel_initializer='he_uniform', name='%s_sc_conv' % name)(tf.expand_dims(features, axis=2))
        if with_bn:
